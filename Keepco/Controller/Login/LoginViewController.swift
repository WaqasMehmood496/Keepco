    ...
    func LoginApi() {
        showHUDView(hudIV: .indeterminate, text: .process) { (hud) in
            ...
            if Connectivity.isConnectedToNetwork(){
                ...
                self.dataDic["user_name"] = self.txtLogin.text!
                self.dataDic["user_password"] = self.txtPassword.text!
                self.callWebService(.login, hud: hud)
            }else{
                hud.textLabel.text = "You are not connected to the internet please check you connection and try again"
...
extension LoginViewController:WebServiceResponseDelegate {
    ...
    func webServiceDataParsingOnResponseReceived(url: webserviceUrl?, viewControllerObj: UIViewController?, dataDict: Any, hud: JGProgressHUD) {
        ...
        case .login:
            if let data = dataDict as? NSDictionary {
                if let userData = UserModel(dic: data) {
...
//MARK:- MENU VIEW METHODS EXTENSION
extension LoginViewController {
    func customizeViews() {
