...
class WorkViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        workAddress = UserDefaults.standard.string(forKey: Config.WORK_ADDRESS) ?? ""
    ...
    override func viewWillAppear(_ animated: Bool) {
          getDetails()
        WorkViewController.isActive = true
      }
    
    override func viewWillDisappear(_ animated: Bool) {
        WorkViewController.isActive = false
    }
      
    func getDetails(){
        let defaults = UserDefaults.standard
        workAddress = defaults.string(forKey: Config.WORK_ADDRESS) ?? ""
        workLat = defaults.string(forKey: Config.WORK_LAT) ?? "0"
        workLng = defaults.string(forKey: Config.WORK_LNG) ?? "0"
            ...
            .responseJSON { response in
                ...
                case .success(let data):
                    ...
                    if success == 1 {
                        ...
                        if(language == "en"){
                            self.view.makeToast("Failed to get posts. Have you selected the work address?")
                        }
                        else {
...
//MARK:- Delegate
extension WorkViewController: UsersListDelegate {
...
//            cell.slider.isContinuous = false
            ...
            cell.labelAddress.text = self.workAddress
            ...
            if(post.type == "media"){
                ...
                let tap2: MyTapGesture = MyTapGesture(target: self, action: #selector(WorkViewController.goToProfile(sender:)))
                ...
                let tap1: MyTapGesture = MyTapGesture(target: self, action: #selector(WorkViewController.goToProfile(sender:)))
                ...
                if(!post.image.isEmpty){
                    ...
                    cell.imgPost.kf.setImage(with: url)
                ...
                cell.btnComment.addTarget(self, action: #selector(WorkViewController.commentTapped(sender:)), for: .touchUpInside)
                
                cell.imgMsg.isUserInteractionEnabled = true
                let tap: MyTapGesture = MyTapGesture(target: self, action: #selector(WorkViewController.msgTapped(sender:)))
...
//          performSegue(withIdentifier: "toPost", sender: nil)
        ...
        if(workLat == "0"){
            self.view.makeToast("Please add work address")
            return
        }
        ...
        postController.lat = workLat
        postController.lng = workLng
...
//MARK:- Delegate
extension WorkViewController: PostControllerDelegate {
    func newPost() {
        goBack()
        getPosts()
    }
